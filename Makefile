...
.PHONY: build
build:
	${GOBUILD} -o jaeger-vl-$(GOOS)-$(GOARCH) ./cmd/jaeger-vl/main.go
...
.PHONY: run
run:
	docker run --rm --name jaeger -e JAEGER_DISABLED=false -it -p9428:9428 -p16686:16686 -p14250:14250 -p14268:14268 -p6831:6831/udp -v "${PWD}:/data" -e SPAN_STORAGE_TYPE=grpc-plugin jaegertracing/all-in-one:${JAEGER_VERSION} --query.ui-config=/data/jaeger-ui.json --grpc-storage-plugin.binary=/data/jaeger-vl-$(GOOS)-$(GOARCH) --grpc-storage-plugin.configuration-file=/data/config.yaml --grpc-storage-plugin.log-level=debug
...
fmt:
	go fmt ./...
	goimports -w -local github.com/jaegertracing/jaeger-vl ./
...
integration-test: build
	STORAGE=grpc-plugin \
	PLUGIN_BINARY_PATH=$(PWD)/jaeger-vl-$(GOOS)-$(GOARCH) \
	PLUGIN_CONFIG_PATH=$(PWD)/integration/config-local.yaml \
	go test ./integration

.PHONY: tar
tar:
	tar -czvf jaeger-vl-$(GOOS)-$(GOARCH).tar.gz  jaeger-vl-$(GOOS)-$(GOARCH) config.yaml
